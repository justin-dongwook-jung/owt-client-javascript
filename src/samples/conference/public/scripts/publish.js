...
const createVideoBox = (userId, stream) => {
    ...
    videoBtn.className = 'userBtn';
    videoBtn.id = 'videoBtn-'+userId;

    let videoBtnText = document.createTextNode(userId);
    videoBtn.appendChild(videoBtnText);
    videoContainner.appendChild(videoBtn);
...
var send = function (method, entity, body, okCallback, errCallback) {
    ...
    if (body !== undefined) {
        printLog(method, JSON.stringify(body));
        req.send(JSON.stringify(body));
    } else {
        printLog(method, entity);
        req.send();
    }
};
...
function subs(stream, userId) {
    client.subscribe(stream, {
        ...
        .then((subscription) => {
            ...
            console.log(`${userId} stream : `, stream);
            let subscribeVideoBox = document.getElementById('multiVideo-'+userId);
            subscribeVideoBox.srcObject = stream.mediaStream;
        }, (err) => {
...
function clientEvent(client) {
    let streamaddedListener = (eve) => {
        ...
        console.log(`new stream added`, remoteStream);
        let participantUserId = idToUserId[remoteStream.origin];

        if(remoteStream.origin === localId) return;

        remoteStreamMap.set(remoteStream.id, remoteStream);
        if (remoteStream.origin) {
            if(hasForward){
                let subscribeBtn = document.getElementById('videoBtn-'+participantUserId);
                if(!subscribeBtn) return;
                subscribeBtn.addEventListener('click', () => {
                    subscribeBtn.disabled = true;
                    subs(remoteStream, participantUserId);
                })
            }
        }
    },
        participantjoinedListener = (eve) => {
            ...
            idToUserId[participant.id] = participant.userId;
            createVideoBox(participant.userId);            
        },
...
function getTrxID(size) {
	var charSet = 'abcde123456789';
	var randomString = '';
...
function printLog(type, contents) {
    let contentsJson = typeof contents === 'string'? contents : JSON.stringify(contents);
...
window.onload = function () {
    ...
    let nickname = getTrxID(4);
    isJoin && createToken(defaultRoomId, nickname, role, (token) => {
        client.join(token)
            .then((resp) => {
                ...
                remoteStreams.forEach((stream) => {
                    ...
                    if (stream.origin && hasForward) {
                        let participantUserId = idToUserId[stream.origin];
                        createVideoBox(participantUserId);
                        let subscribeBtn = document.getElementById('videoBtn-'+participantUserId);
                        subscribeBtn.addEventListener('click', () => {
                            subs(stream, participantUserId)
                        })
                    }
                })
                ...
                MediaStreamFactory.createMediaStream({
                    ...
                    .then((localStream) => {
                        ...
                        createVideoBox(`local(${nickname})`, localStream.mediaStream);
...
window.onbeforeunload = function (event) {
    ...
    for (var i in conference.remoteStreams) {
        if (conference.remoteStreams.hasOwnProperty(i)) {
            ...
            delete conference.remoteStrreams[i];
        }
    }*/
